...
assert(
  bulkRoute.includes("course?.active === false") &&
    bulkRoute.includes("هذه الدورة موقوفة عن التسجيل حالياً"),
  "API التسجيل الجماعي يرفض الدورة الموقوفة عن التسجيل",
);
assert(
  bulkRoute.includes("activeLinks.length > 1") &&
    bulkRoute.includes("أكثر من فصل نشط"),
  "API التسجيل الجماعي يرفض تعارض أكثر من فصل نشط للدورة",
);
assert(
  bulkRoute.includes("activeLinks.length === 0") &&
    bulkRoute.includes("warnings.push") &&
    bulkRoute.includes("الطالب سيُسجل بدون فرص"),
  "API التسجيل الجماعي لا يخفي حالة عدم وجود فصل نشط ويرجع تنبيه واضح",
);
assert(
  bulkRoute.includes("include:") &&
    bulkRoute.includes(
      "chapter: { select: { id: true, name: true, opportunities: true } }",
    ) &&
    bulkRoute.includes("baseOpportunities: opportunities"),
...
assert(
  ...
  ) && !bulkRoute.includes("asText(payload.opportunities"),
...
assert(
  bulkRoute.includes("withSerializableTransaction") &&
    bulkRoute.includes("freshCourses") &&
    bulkRoute.includes("freshActiveLinks") &&
    bulkRoute.includes("executionWarnings") &&
    serializableTransaction.includes("TransactionIsolationLevel.Serializable"),
  "التسجيل الجماعي يعيد قراءة الدورات والفصول داخل transaction ذرية قبل إنشاء أي طالب",
);
assert(
...
assert(
  bulkView.includes("studentRegisterApi.context()") &&
    bulkView.includes("StudentRegisterContextResponse"),
  "واجهة التسجيل الجماعي تحمل سياق الدورات والفصول من API قاعدة البيانات",
);
assert(
  !bulkView.includes("const { students, courses, loadFromServer") &&
    !bulkView.includes("findCourse(courses,"),
...
assert(
  bulkView.includes("course.active === false") &&
    bulkView.includes("موقوفة عن التسجيل") &&
    bulkView.includes("courseRow.activeChapterCount > 1"),
...
assert(
  packageJson.scripts?.["test:bulk-register-integrity"] ===
    "node scripts/test-bulk-register-integrity.mjs",
