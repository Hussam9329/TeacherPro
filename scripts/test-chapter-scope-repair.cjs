...
(async () => {
  ...
  const plan = {
    repairId: 'chapter_scope_20260910_v1', expected,
  ...
  const audit = JSON.parse((await db.query(`SELECT details FROM "AuditLog" WHERE id='chapter_scope_20260910_v1_s'`)).rows[0].details);
  ...
  assert.equal((await db.query(`SELECT count(*)::int n FROM "AuditLog" WHERE id LIKE 'chapter_scope_20260910_v1%'`)).rows[0].n, 2);
  await db.close();
