...
check(
  studentActionRoute.includes("links.length !== 1") &&
    studentActionRoute.includes(
...
check(
  bulkAdjustRoute.includes('body.mode !== "filter"') &&
    bulkAdjustRoute.includes("composeStudentWhere(filters)") &&
...
check(
  opportunityLogsRoute.includes(
    "studentId = String(searchParams.get('studentId')",
  ) &&
...
check(
  opportunitySnapshotServer.includes(
    "Current balance always comes from Student.opportunities",
  ) &&
    opportunitySnapshotServer.includes(
      "const opportunityLimit = activeChapter?.opportunities ?? null",
    ) &&
    opportunitySnapshotServer.includes(
      "attachStudentOpportunitySnapshotsWithClient",
    ),
...
check(
  opportunityBalance.includes("formatOpportunityBalance") &&
    opportunityBalance.includes(
      'Object.prototype.hasOwnProperty.call(source, "opportunityLimit")',
    ),
