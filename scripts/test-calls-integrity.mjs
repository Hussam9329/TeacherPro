...
const files = {
  ...
  callUniqueMigration:
    "prisma/migrations/20260708162000_student_call_unique_key/migration.sql",
...
assert(
  followUp.includes("callCourseExamsApi") &&
    followUp.includes("callCourseExamsFromDb"),
  "قائمة امتحانات تبويبة المكالمات تأتي من API قاعدة البيانات",
);
assert(
  followUp.includes("studentCallApi.upsert") &&
    callsRoute.includes("db.$transaction"),
...
assert(
  callUniqueMigration.includes('COALESCE("examId",') &&
    callUniqueMigration.includes(
      "StudentCall_studentId_examId_category_coalesced_key",
    ),
...
assert(
  profileLog.includes("const candidateExams = await db.exam.findMany") &&
    profileLog.includes("const examStates = []") &&
    profileLog.includes("exams,") &&
    profileDialog.includes("databaseExams") &&
    profileDialog.includes("profileExams"),
  "ملف الطالب المفتوح من المكالمات يجلب جميع امتحانات دورة وموقع الطالب من قاعدة البيانات حتى بلا درجة",
);
assert(
