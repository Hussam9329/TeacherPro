...
must(
  ...
  "يجب إظهار قاعدة تقديم تاريخ التسجيل واحتساب الدرجة في ورقة الإدخال.",
);

// طلب صاحب النظام: لا تُعرض ملاحظة «محفوظة للمتابعة فقط» في سجل الدرجات إطلاقاً.
must(
  !gradeRecords.includes("محفوظة للمتابعة فقط") &&
    !gradeRecords.includes("تاريخ الامتحان يسبق تاريخ"),
  "سجل الدرجات خالٍ من ملاحظة «محفوظة للمتابعة فقط»",
  "ملاحظة ما قبل التسجيل يجب ألا تظهر في سجل الدرجات.",
);
