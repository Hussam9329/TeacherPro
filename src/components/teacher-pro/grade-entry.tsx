...
export function GradeEntryView() {
  const { exams, students, grades, addGrade, updateGrade, courseName, classification } = useTeacherStore();
  ...
  const getGrade = (studentId: string) => grades.find(g => g.studentId === studentId && g.examId === selectedExamId);
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <Select value={selectedExamId} onValueChange={setSelectedExamId}>
                ...
                <SelectContent>
                  {activeExams.map(e => (
                    <SelectItem key={e.id} value={e.id}>{e.name} ({e.type}) - {e.date}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
            ...
            {selectedExam && (
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 p-3 rounded-2xl bg-muted/60">
                <div><span className="text-muted-foreground text-xs">النوع:</span> <Badge>{selectedExam.type}</Badge></div>
                <div><span className="text-muted-foreground text-xs">النجاح:</span> {selectedExam.passMark}</div>
                <div><span className="text-muted-foreground text-xs">الخصم:</span> {selectedExam.discountMark}</div>
                <div><span className="text-muted-foreground text-xs">الفصل:</span> {selectedExam.dismissalGrade || 'لا يوجد'}</div>
              </div>
            )}
          </div>
        </CardContent>
          ...
          <CardContent>
            <div className="space-y-2">
              {examStudents.map(student => {
                ...
                return (
                  <div key={student.id} className="p-3 rounded-2xl border bg-card/80 shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:border-primary/30 hover:shadow-lg">
                    <div className="flex items-center justify-between gap-3">
                      ...
                      {grade && !isEditing && (
                        <div className="flex items-center gap-2">
                          ...
                          {grade.score !== null && (
                            <span className="text-sm font-bold">{grade.score}/{selectedExam.fullMark}</span>
                          )}
                        </div>
                      )}
                      ...
                      {isEditing ? (
                        ...
                        <Button size="sm" variant="outline" onClick={() => startEdit(student.id)}>
                          {grade ? 'تعديل' : 'إدخال'}
                        </Button>
                      )}
                    </div>
