...
import { AlertCircle, BookOpen, CheckCheck, ChevronDown, Loader2, LockKeyhole, RefreshCw, Search, UserRound, X } from "lucide-react";
...
export function CodeClosuresDialog({ open, onOpenChange, canManage }: Props) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="tp-code-closures" dir="rtl">
        <div className="tp-code-closures__hero">
          <span className="tp-code-closures__hero-icon" aria-hidden="true"><LockKeyhole /></span>
          <DialogHeader className="tp-code-closures__heading">
            <DialogTitle>اغلاق الكودات</DialogTitle>
          </DialogHeader>
        </div>

        <div className="tp-code-closures__controls">
          <div role="group" aria-label="حالة اغلاق الكود" className="tp-code-closures__filters" dir="ltr">
            {([
              ["all", "All", "الكل"],
              ["checked", "Checked", "مغلقة"],
              ["unchecked", "unChecked", "بانتظار الإغلاق"],
              ...
              <Button
                ...
                className="tp-code-closures__filter"
              >
                <span className="tp-code-closures__filter-label">{label}</span>
                <strong id={`${filterId}-${value}-count`} className="tp-code-closures__filter-count">{loaded ? counts[value] : "—"}</strong>
                <span id={`${filterId}-${value}-description`} className="tp-code-closures__filter-description" dir="rtl"><i aria-hidden="true" />{description}</span>
              </Button>
            ))}
          </div>

          <div className="tp-code-closures__search-row">
            <label className="tp-code-closures__field">
              <span>بحث عن طالب</span>
              <div className="tp-code-closures__input-wrap">
                <Search aria-hidden="true" />
                <Input value={search} onChange={(event) => setSearch(event.target.value)} aria-label="بحث في اغلاق الكودات" placeholder="اسم الطالب أو الكود" className="tp-code-closures__search" />
              </div>
            </label>
            <label className="tp-code-closures__field">
              <span>الدورة</span>
              <div className="tp-code-closures__select-wrap">
                ...
                <ChevronDown className="tp-code-closures__chevron" aria-hidden="true" />
              </div>
            </label>
          </div>
        </div>

        <div className="tp-code-closures__toolbar">
          <span className="tp-code-closures__result-count" aria-live="polite">
            {loaded ? `المعروض ${visibleStudents.length} من ${scopedStudents.length} طالب مفصول` : "الطلاب المفصولون"}
          </span>
          <div className="tp-code-closures__tools">
            {hasFilters && <Button type="button" variant="ghost" size="sm" className="tp-code-closures__clear" onClick={clearFilters}><X className="size-4" aria-hidden="true" />مسح الفلاتر</Button>}
            <Button type="button" variant="outline" size="sm" className="tp-code-closures__refresh" disabled={loading || pendingIds.size > 0} onClick={() => void refresh()}>
              <RefreshCw className={`size-4 ${loading ? "animate-spin motion-reduce:animate-none" : ""}`} aria-hidden="true" />تحديث
            </Button>
          </div>
        </div>

        {error && <div role="alert" className="tp-code-closures__error"><AlertCircle aria-hidden="true" /><p>{error}</p></div>}
        {pendingIds.size > 0 && <p role="status" className="tp-code-closures__saving"><Loader2 className="size-4 animate-spin motion-reduce:animate-none" aria-hidden="true" />جاري حفظ اغلاق الكود...</p>}

        <div className="tp-code-closures__results">
          {!loaded && loading ? (
            <div className="tp-code-closures__empty" role="status">
              <span className="tp-code-closures__empty-icon"><Loader2 className="animate-spin motion-reduce:animate-none" aria-hidden="true" /></span>
              <p>جاري تحميل الطلاب...</p>
            </div>
          ) : loaded && visibleStudents.length === 0 ? (
            <div className="tp-code-closures__empty" data-complete={!search.trim() && !courseId && statusFilter === "unchecked" && pendingIds.size === 0}>
              <span className="tp-code-closures__empty-icon">{search.trim() || courseId ? <Search aria-hidden="true" /> : <CheckCheck aria-hidden="true" />}</span>
            ...
            <div className="tp-code-closures__list">
              {visibleStudents.length > 0 && <div aria-hidden="true" className="tp-code-closures__column-headings"><span>الطالب والكود</span><span>الدورة</span><span>سبب الفصل</span><span>اغلاق كود</span></div>}
              {visibleStudents.map((student) => (
                <article key={student.id} className="tp-code-closures__student" data-checked={student.dismissedChecked}>
                  <div className="tp-code-closures__identity">
                    <span className="tp-code-closures__avatar" aria-hidden="true"><UserRound /></span>
                    <div className="tp-code-closures__student-info">
                      <p className="tp-code-closures__name">{student.name}</p>
                      <span dir="ltr" className="tp-code-closures__code">{student.code}</span>
                    </div>
                  </div>
                  <p className="tp-code-closures__course"><BookOpen aria-hidden="true" /><span>{student.course?.name || "—"}</span></p>
                  <div className="tp-code-closures__reason"><span>سبب الفصل</span><p>{student.dismissalReason || "—"}</p></div>
                  <label className="tp-code-closures__action" data-disabled={!canManage || pendingIds.has(student.id)}>
                    <Checkbox
                      ...
                      className="tp-code-closures__checkbox"
                    />
