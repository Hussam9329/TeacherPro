...
export function GroupsView() {
  const { groups, courses, addGroup, updateGroup, toggleGroup, deleteGroup, courseName } = useTeacherStore();
  const [name, setName] = useState('');
  const [courseId, setCourseId] = useState('');
  const [electronicGroup, setElectronicGroup] = useState('');
  const [editDialog, setEditDialog] = useState({ open: false, id: '', groupName: '', electronic: '' });
  const [deleteDialog, setDeleteDialog] = useState({ open: false, id: '', groupName: '' });

  const openEditDialog = (id: string, oldName: string, oldElectronic: string) => {
    setEditDialog({ open: true, id, groupName: oldName, electronic: oldElectronic });
  };
  const handleEditSave = () => {
    if (!editDialog.groupName.trim()) { toast.error('يرجى إدخال اسم الكروب'); return; }
    updateGroup(editDialog.id, { name: editDialog.groupName.trim(), electronicGroup: editDialog.electronic.trim() });
  ...
  const handleDeleteConfirm = () => {
    ...
    ok ? toast.success('تم حذف الكروب') : toast.error('لا يمكن حذف الكروب لأنه مرتبط ببيانات أخرى');
    setDeleteDialog({ open: false, id: '', groupName: '' });
  };

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim()) { toast.error('يرجى إدخال اسم الكروب'); return; }
    if (!courseId) { toast.error('يرجى اختيار الدورة'); return; }
  ...
  return (
    <div className="space-y-6">
      <Card>
        ...
        <CardContent>
          <p className="text-sm text-muted-foreground mb-4">إدارة الكروبات المرتبطة بالدورات</p>
        </CardContent>
      </Card>
          ...
          <CardContent>
            <div className="space-y-3 max-h-[500px] overflow-y-auto">
              {groups.length === 0 ? (
                ...
                groups.map((group) => (
                  <div key={group.id} className="p-4 rounded-2xl border bg-card/80 shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:border-primary/30 hover:shadow-lg">
                    ...
                    <div className="grid grid-cols-2 gap-2 text-sm">
                      <div>
                        ...
                        <p className="font-medium">{courseName(group.courseId)}</p>
                      </div>
                      <div>
                        <span className="text-muted-foreground text-xs">الكروب الإلكتروني</span>
                    ...
                    <div className="flex gap-2 mt-3">
                      <Button
                        ...
                        onClick={() => {
                          ...
                          toast.success(group.active ? 'تم تعطيل الكروب' : 'تم تفعيل الكروب');
                        }}
                      >
                        {group.active ? 'تعطيل' : 'تفعيل'}
                      </Button>
                      <Button variant="secondary" size="sm" onClick={() => openEditDialog(group.id, group.name, group.electronicGroup)}>تعديل</Button>
                      <Button variant="destructive" size="sm" onClick={() => openDeleteDialog(group.id, group.name)}>حذف</Button>
                    </div>
                  </div>
                ))
              )}
        ...
        <AlertDialogContent dir="rtl">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              هل تريد حذف الكروب &quot;{deleteDialog.groupName}&quot;؟ لا يمكن حذف كروب مرتبط بطلاب أو امتحانات.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
