...
export function ExamNewView() {
  const { courses, groups, sites, exams, students, addExam, toggleExam, toggleAttendance, closeAttendance, courseName, groupName } = useTeacherStore();

  const [form, setForm] = useState({
    name: '', type: 'يومي' as 'يومي' | 'تراكمي' | 'فاينل',
    courseIds: [] as string[], mainSites: [] as string[], groupIds: [] as string[],
    date: new Date().toISOString().slice(0, 10),
    fullMark: 100, passMark: 60, discountMark: 45,
    opportunitiesPenaltyNum: 1, dismissalGrade: '',
  });
  ...
  const availableMainSites = useMemo(() => {
    const courseSites = sites
      .filter(s => s.active && (form.courseIds.length === 0 || form.courseIds.includes(s.courseId)))
      .map(s => s.main);
  ...
  const allCoursesSelected = courses.length > 0 && form.courseIds.length === courses.length;
  const allMainSitesSelected = availableMainSites.length > 0 && form.mainSites.length === availableMainSites.length;
  const allGroupsSelected = availableGroups.length > 0 && form.groupIds.length === availableGroups.length;

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.name.trim()) { toast.error('يرجى إدخال اسم الامتحان'); return; }
    if (form.courseIds.length === 0) { toast.error('يرجى اختيار دورة واحدة على الأقل'); return; }
    if (form.mainSites.length === 0) { toast.error('يرجى اختيار منطقة واحدة على الأقل أو اختيار الكل'); return; }
    if (availableGroups.length > 0 && form.groupIds.length === 0) { toast.error('يرجى اختيار كروب واحد على الأقل أو اختيار الكل'); return; }
    ...
    addExam(examData);
    setForm({
      name: '', type: 'يومي', courseIds: [], mainSites: [], groupIds: [],
      date: new Date().toISOString().slice(0, 10),
      fullMark: 100, passMark: 60, discountMark: 45,
      opportunitiesPenaltyNum: 1, dismissalGrade: '',
    });
  ...
  const toggleCourseSelection = (courseId: string) => {
    setForm(prev => ({ ...prev, courseIds: toggleSelection(prev.courseIds, courseId), groupIds: [] }));
  };

  const toggleAllCourses = () => {
    setForm(prev => ({ ...prev, courseIds: allCoursesSelected ? [] : courses.map(c => c.id), groupIds: [] }));
  };

  const toggleMainSiteSelection = (mainSite: string) => {
    setForm(prev => ({ ...prev, mainSites: toggleSelection(prev.mainSites, mainSite) }));
  };

  const toggleAllMainSites = () => {
    setForm(prev => ({ ...prev, mainSites: allMainSitesSelected ? [] : [...availableMainSites] }));
  };

  const toggleGroupSelection = (groupId: string) => {
    setForm(prev => ({ ...prev, groupIds: toggleSelection(prev.groupIds, groupId) }));
  };

  const toggleAllGroups = () => {
    setForm(prev => ({ ...prev, groupIds: allGroupsSelected ? [] : availableGroups.map(g => g.id) }));
  };
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-3">
            ...
            <div className="md:col-span-2 lg:col-span-3 space-y-3">
              <Button type="submit" className="w-full">إضافة الامتحان</Button>
            </div>
          </form>
        </CardContent>
      </Card>
        ...
        <CardContent>
          <div className="space-y-3">
            {exams.map(exam => {
              ...
              return (
                <div key={exam.id} className="p-4 rounded-2xl border bg-card/80 shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:border-primary/30 hover:shadow-lg">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-sm mb-2">
                    <div><span className="text-muted-foreground text-xs">النجاح:</span> {exam.passMark}</div>
                    <div><span className="text-muted-foreground text-xs">الخصم:</span> {exam.discountMark}</div>
                    <div><span className="text-muted-foreground text-xs">الحضور:</span> {exam.attendance.length} طالب</div>
                    <div><span className="text-muted-foreground text-xs">الدورات:</span> {exam.courseIds.map(id => courseName(id)).join(', ')}</div>
                    <div><span className="text-muted-foreground text-xs">المناطق:</span> {examMainSites.join(', ') || 'الكل'}</div>
                    <div><span className="text-muted-foreground text-xs">الكروبات:</span> {examGroupIds.map(id => groupName(id)).join(', ') || 'الكل'}</div>
                  </div>
                  <div className="flex gap-2">
                    <Button variant="outline" size="sm" onClick={() => toggleExam(exam.id)}>
                      {exam.active ? 'تعطيل' : 'تفعيل'}
                    </Button>
                    <Button variant="outline" size="sm" onClick={() => setSelectedExamId(exam.id)}>
          ...
          <CardHeader className="flex flex-row items-center justify-between">
            ...
            <div className="flex gap-2">
              {!selectedExam.attendanceClosed && (
                <Button size="sm" onClick={() => { closeAttendance(selectedExam.id); toast.success('تم إغلاق الحضور'); }}>
                  إغلاق الحضور
                </Button>
              )}
          ...
          <CardContent>
            <div className="space-y-2">
              {selectedExamStudents.length === 0 ? (
                <p className="empty-state">لا يوجد طلاب مطابقون للدورات والمناطق والكروبات المختارة.</p>
              ) : selectedExamStudents.map(s => (
                <div key={s.id} className="flex items-center justify-between p-2 rounded-xl bg-muted/60">
