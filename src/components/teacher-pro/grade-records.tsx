...
export function GradeRecordsView() {
  const { grades, exams, students, courses, updateGrade, deleteGrade, classification } = useTeacherStore();
  ...
  const filtered = useMemo(() => {
    return grades.filter(g => {
      ...
      if (search && !student.name.includes(search) && !student.code.includes(search) && !student.telegram.includes(search)) return false;
  ...
  const handleEditGrade = (gradeId: string) => {
    ...
    if (!grade) return;
    const nextStatus = prompt('الحالة: درجة / غائب / مجاز / غش', grade.status) as 'درجة' | 'غائب' | 'مجاز' | 'غش' | null;
    if (!nextStatus || !['درجة', 'غائب', 'مجاز', 'غش'].includes(nextStatus)) return;
    const nextScore = nextStatus === 'درجة' ? Number(prompt('الدرجة', String(grade.score ?? 0)) || 0) : null;
  ...
  const handleDeleteGrade = (gradeId: string) => {
    if (!confirm('هل تريد حذف سجل الدرجة؟')) return;
    const ok = deleteGrade(gradeId);
    ok ? toast.success('تم حذف الدرجة') : toast.error('تعذر حذف الدرجة');
  };

  const exportCSV = () => {
    const headers = ['الطالب', 'الكود', 'التلكرام', 'الامتحان', 'الحالة', 'الدرجة', 'التصنيف', 'محاسبة', 'ملاحظات'];
    const rows = filtered.map(g => {
      const student = students.find(s => s.id === g.studentId);
      const exam = exams.find(e => e.id === g.examId);
      const cls = classification(g, exam!);
      return [
        student?.name || '', student?.code || '', student?.telegram || '',
        exam?.name || '', g.status, g.score?.toString() || '',
        cls.text, g.accountingChecked ? 'نعم' : 'لا', g.notes,
      ].map(v => `"${v}"`).join(',');
    });
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between text-sm text-muted-foreground">
        ...
        <div className="flex items-center gap-2">
          ...
          <Select value={String(pageSize)} onValueChange={v => { setPageSize(Number(v)); setPage(1); }}>
            <SelectTrigger id="grade-records-pageSize" className="w-20 h-8"><SelectValue /></SelectTrigger>
            <SelectContent>
      ...
      <div className="space-y-2">
        {paged.map(g => {
          ...
          return (
            <div key={g.id} className="flex items-center justify-between gap-3 p-3 rounded-2xl border bg-card/80 shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:border-primary/30 hover:shadow-lg">
              ...
              <div className="flex items-center gap-2">
                {g.score !== null && <span className="font-bold">{g.score}/{exam.fullMark}</span>}
                <Badge variant={
                  cls.type === 'ok' ? 'default' :
                  cls.type === 'danger' ? 'destructive' :
                  cls.type === 'warn' ? 'secondary' :
                  'outline'
                }>
                  {cls.text}
                </Badge>
                {g.accountingChecked && <Badge variant="outline">محاسبة</Badge>}
                <Button variant="secondary" size="sm" onClick={() => handleEditGrade(g.id)}>تعديل</Button>
                <Button variant="destructive" size="sm" onClick={() => handleDeleteGrade(g.id)}>حذف</Button>
              </div>
            </div>
          );
        })}
