...
export function SitesView() {
  const { sites, courses, addSite, updateSite, toggleSite, deleteSite, courseName } = useTeacherStore();
  const [courseId, setCourseId] = useState('');
  const [main, setMain] = useState('بغداد');
  const [sub, setSub] = useState('');
  const [editDialog, setEditDialog] = useState({ open: false, id: '', mainSite: '', subSite: '' });
  const [deleteDialog, setDeleteDialog] = useState({ open: false, id: '', siteName: '' });
  ...
  const handleEditSave = () => {
    if (!editDialog.mainSite.trim()) { toast.error('يرجى إدخال الموقع الرئيسي'); return; }
    if (!editDialog.subSite.trim()) { toast.error('يرجى إدخال الموقع الفرعي'); return; }
    updateSite(editDialog.id, { main: editDialog.mainSite.trim(), sub: editDialog.subSite.trim() });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!sub.trim()) { toast.error('يرجى إدخال الموقع الفرعي'); return; }
  ...
  return (
    <div className="space-y-6">
      <Card>
        ...
        <CardContent>
          <p className="text-sm text-muted-foreground">الموقع الرئيسي ثابت من قائمة المحافظات العراقية الـ18 مع خيار أونلاين، والمستخدم يضيف المناطق الفرعية فقط لكل موقع رئيسي.</p>
        </CardContent>
      </Card>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Select value={main} onValueChange={setMain}>
                  <SelectTrigger id="site-mainSite"><SelectValue /></SelectTrigger>
                  <SelectContent>
          ...
          <CardContent>
            <div className="space-y-3 max-h-[500px] overflow-y-auto">
              {sites.length === 0 ? (
                ...
                sites.map((site) => (
                  <div key={site.id} className="p-4 rounded-2xl border bg-card/80 shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:border-primary/30 hover:shadow-lg">
                    <div className="flex items-center justify-between mb-2">
                      <p className="font-bold">{site.main} - {site.sub}</p>
                      <Badge variant={site.active ? 'default' : 'secondary'}>
                        {site.active ? 'فعال' : 'معطل'}
                      </Badge>
                    </div>
                    <p className="text-sm text-muted-foreground mb-3">{courseName(site.courseId)}</p>
                    <div className="flex gap-2">
                      <Button
                        ...
                        onClick={() => {
                          toggleSite(site.id);
                          toast.success(site.active ? 'تم تعطيل الموقع' : 'تم تفعيل الموقع');
                        }}
                      >
                        {site.active ? 'تعطيل' : 'تفعيل'}
                      </Button>
                      <Button variant="secondary" size="sm" onClick={() => openEditDialog(site.id, site.main, site.sub)}>تعديل</Button>
                      <Button variant="destructive" size="sm" onClick={() => openDeleteDialog(site.id, `${site.main} - ${site.sub}`)}>حذف</Button>
                    </div>
                  </div>
                ))
              )}
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <Select value={editDialog.mainSite} onValueChange={value => setEditDialog(prev => ({ ...prev, mainSite: value }))}>
                <SelectTrigger id="edit-site-mainSite"><SelectValue /></SelectTrigger>
                <SelectContent>
                  {MAIN_SITE_OPTIONS.map((site) => (
                    <SelectItem key={site} value={site}>{site}</SelectItem>
                  ))}
