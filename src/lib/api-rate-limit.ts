...
async function redisCommand<T = unknown>(
  ...
  try {
    const response = await fetch(
      `${UPSTASH_URL!}/${args.map((arg) => encodeURIComponent(String(arg))).join("/")}`,
      {
    ...
    if (!response.ok) {
      if (!redisWarningLogged) {
        console.warn(
          "[api-rate-limit] Upstash request failed:",
          response.status,
...
function distributedLimiterUnavailableResponse() {
  return NextResponse.json(
    {
      error:
        "الحماية الموزعة للطلبات غير متاحة حالياً. لم يُنفذ الطلب لحماية النظام من تجاوز الحدود بين نسخ الخادم.",
    },
...
export async function checkApiRateLimit(
  ...
  const message =
    options.message ||
    "طلبات كثيرة خلال مدة قصيرة. انتظر قليلاً ثم حاول مرة أخرى.";
    ...
    if (count !== null) {
      ...
      if (Number(count) > options.limit) {
        ...
        return rateLimitResponse(
          message,
          ttl && ttl > 0 ? ttl : options.windowSeconds,
        );
      }
      return null;
    }
  ...
  if (readEnv("NODE_ENV") === "production") {
    return distributedLimiterUnavailableResponse();
  }
...
export const API_RATE_LIMITS = {
  backup: {
    ...
    windowSeconds: 5 * 60,
    message:
      "طلبات النسخ الاحتياطي كثيرة خلال مدة قصيرة. انتظر قليلاً ثم حاول مرة أخرى.",
  },
  bulkOpportunities: {
    key: "bulk-opportunities",
    limit: 15,
    windowSeconds: 5 * 60,
    message:
      "طلبات تعديل الفرص الجماعية كثيرة خلال مدة قصيرة. انتظر قليلاً ثم حاول مرة أخرى.",
  },
  bulkStudents: {
    key: "bulk-students",
    limit: 10,
    windowSeconds: 10 * 60,
    message:
      "طلبات الإضافة الجماعية للطلاب كثيرة خلال مدة قصيرة. انتظر قليلاً ثم حاول مرة أخرى.",
  },
  studentOpportunitySync: {
    key: "student-opportunity-sync",
    limit: 10,
    windowSeconds: 10 * 60,
    message:
      "طلبات مزامنة فرص الطلاب كثيرة خلال مدة قصيرة. انتظر قليلاً ثم حاول مرة أخرى.",
  },
  adminHeavy: {
    key: "admin-heavy",
    limit: 10,
    windowSeconds: 10 * 60,
    message:
      "طلبات إدارية ثقيلة كثيرة خلال مدة قصيرة. انتظر قليلاً ثم حاول مرة أخرى.",
  },
} as const;
