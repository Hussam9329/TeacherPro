...
export async function annotateGradeSettlementEffects<T extends { id: string; studentId: string; examId: string; notes?: string | null }>(grades: T[]): Promise<void> {
 ...
 const [logs, students, exams, links] = await Promise.all([
  ...
  db.exam.findMany({ where: { id: { in: [...new Set(grades.map(g => g.examId))] } }, select: { id: true, date: true, examCourses: { select: { courseId: true, chapterId: true } } } }),
 ...
 const courseByStudent = new Map(students.map(s => [s.id, s.courseId]));
 ...
 for (const grade of grades) {
  const reason = gradeSettlementExclusion(grade, examsById.get(grade.examId) || {}, logsByStudent.get(grade.studentId), chapterByStudent.get(grade.studentId), courseByStudent.get(grade.studentId));
