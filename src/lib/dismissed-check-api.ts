...
export type DismissedCheckSnapshot = { id: string; status: string; dismissedChecked: boolean };
...
export async function readDismissedChecks(ids: string[], signal?: AbortSignal): Promise<DismissedCheckSnapshot[]> {
  ...
  return withReadDeadline(async (requestSignal) => {
    ...
    if (!response.ok) throw new Error(data.error || "تعذر تحميل تأشير الطلاب.");
    return data.students;
  }, signal, 15_000);
}

export async function saveDismissedCheck(studentId: string, checked: boolean, expectedChecked: boolean): Promise<DismissedCheckSnapshot> {
  ...
  const response = await fetch("/api/students/dismissed-check", {
    ...
    body: JSON.stringify({ studentId, checked, expectedChecked }),
  });
  const data = await response.json();
  if (!response.ok) throw new Error(data.error || "تعذر حفظ التأشير.");
  return data.student;
}
