...
function includesSensitivePermission(value: unknown): boolean {
  return parsePermissionIds(value).some((permission) =>
    SENSITIVE_PERMISSION_IDS.has(permission),
  );
}

function samePermissionIds(a: unknown, b: unknown): boolean {
  const normalize = (value: unknown) =>
    Array.from(new Set(parsePermissionIds(value)))
      .sort()
      .join("\u0000");
...
function validateSensitiveUserChanges(
  ...
  const requestedUsername = String(payload.username ?? "")
    .trim()
    .toLowerCase();
  const requestedRoleId = String(payload.roleId ?? "");
  const targetIsAdmin = Boolean(
    existingUser &&
    (isPrimaryAdminUser(existingUser) || isAdminRoleUser(existingUser)),
  );

  if (
    !actorIsOwner &&
    (targetIsAdmin ||
      requestedUsername === ADMIN_USERNAME ||
      requestedRoleId === ADMIN_ROLE_ID)
  ) {
    return forbiddenSecurity(
      "لا يمكن لمدير حسابات عادي إنشاء أو تعديل حساب مدير النظام.",
    );
  }
  ...
  if (
    ...
    includesSensitivePermission(payload.permissions)
  ) {
    return forbiddenSecurity(
      "لا يمكن منح صلاحيات حساسة إلا من حساب admin الرئيسي.",
    );
  }
  ...
  if (principal.id === String(payload.id || "") && existingUser) {
    const roleIdChanged =
      ...
      !sameOptionalString(payload.roleId, existingUser.roleId);
    const roleNameChanged =
      payload.role !== undefined &&
      !sameOptionalString(payload.role, existingUser.role);
    const permissionsChanged =
      payload.permissions !== undefined &&
      !samePermissionIds(payload.permissions, existingUser.permissions);
    const activeChanged =
      payload.active !== undefined &&
      !sameOptionalBoolean(payload.active, existingUser.active);
    ...
    if (
      ...
      return forbiddenSecurity(
        "لا يمكن تعديل دورك أو صلاحياتك أو حالة حسابك من نفس الجلسة.",
      );
    }
  }
...
async function readAssignableRole(principal: AuthPrincipal, roleId: unknown) {
  ...
  if (!role)
    return {
      role: null,
      error: validationError("الدور غير موجود أو تم حذفه.", 404),
    };
  if (!isOwner(principal) && includesSensitivePermission(role.permissions)) {
    return {
      role: null,
      error: forbiddenSecurity(
        "لا يمكن تعيين دور يحتوي صلاحيات حساسة إلا من حساب admin الرئيسي.",
      ),
    };
  }
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!passwordPolicy.ok) return validationError(passwordPolicy.message);
...
export async function PUT(req: NextRequest) {
  ...
  try {
    ...
    const securityError = validateSensitiveUserChanges(
      principal,
      { id, ...data },
      existingUser,
    );
    ...
    const roleAssignment =
      data.roleId !== undefined
        ? await readAssignableRole(principal, data.roleId)
    ...
    if (updateData.permissions !== undefined)
      updateData.permissions = normalizePermissions(updateData.permissions);
    ...
    const passwordInput =
      typeof password === "string"
        ...
        : typeof passwordHash === "string"
          ...
          : "";
    if (passwordInput) {
      const passwordPolicy = validatePasswordStrength(passwordInput, {
        username: String(existingUser.username || ""),
        name: String(data.name ?? existingUser.name ?? ""),
      });
      if (!passwordPolicy.ok) return validationError(passwordPolicy.message);
      // passwordHash is accepted only as a legacy field name; its value is always
      // treated as plaintext client input and re-hashed server-side. A caller can
      // never inject a precomputed hash into the database.
...
export async function DELETE(req: NextRequest) {
  ...
  try {
    ...
    if (!id) return validationError("تعذر تحديد المستخدم المطلوب");
    if (id === principal.id)
      return validationError("لا يمكن حذف حسابك الحالي من نفس الجلسة.", 403);
    ...
    if (isPrimaryAdminUser(user) || isAdminRoleUser(user)) {
      return validationError(
        "لا يمكن حذف حساب مدير النظام. عطّل أو عدّل حسابات المستخدمين العادية فقط.",
        403,
      );
    }
    const linkedSheets = await db.correctionSheet.count({
      where: { correctorId: id },
    });
    if (linkedSheets > 0) {
      return validationError(
        "لا يمكن حذف المستخدم لأنه مرتبط بأوراق تصحيح. عطّل الحساب بدلاً من حذفه.",
        409,
      );
    }
