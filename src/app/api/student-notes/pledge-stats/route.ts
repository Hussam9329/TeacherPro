...
function dismissalGroupFromType(type: string | null | undefined): "temporary" | "final" {
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const temporary = dismissedStudents.filter(
      (student) => dismissalGroupFromType(student.dismissalType || "فصل مؤقت") === "temporary",
    ).length;
    const final = dismissedStudents.filter(
      (student) => dismissalGroupFromType(student.dismissalType || "فصل مؤقت") === "final",
    ).length;
    const reactivated = pledgeNotes.filter((note) => note.student?.status && note.student.status !== "مفصول").length;
    ...
    return routeErrorResponse(error, "تعذر تحميل إحصائيات التعهدات من قاعدة البيانات.");
  }
}
