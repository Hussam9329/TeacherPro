...
## 10. الإصلاح الثالث (2026-08-20) — معالجة الطالب المجاز كغائب

### المشكلة الثالثة المُكتشفة
أبلغ المستخدم عن الطالبة «روان ياسر عبدالاله علوان» (BIO-2401): مجازة من الامتحان 16، لكن يظهر عليها "فصل" مع ملاحظة "تسجيل جماعي كغائب للطلاب غير المدخلة درجاتهم".

الفحص الإنتاجي أظهر **نوعين من التناقض**:

| النوع | العدد | الوصف |
|---|---|---|
| (أ) `status='مجاز'` + ملاحظة "تسجيل جماعي كغائب" | **331 صف** | الطالب مجاز فعلاً، لكن الملاحظة تقول إنه غائب |
| (ب) `status='غائب'`/`'غش'` + إجازة فعلية موجودة | **119 صف** | الطالب مجاز، لكن السجل يقول غائب — **الخطر الحقيقي!** |

**النوع (ب) هو الأخطر**: عند تشغيل `recalculateStudentsAcademicState`، الـ grade `status="غائب"` يُعامل معاملة الغياب الحقيقي → خصم فرصة أو **فصل خاطئ**.

### السبب الجذري
في `mark-missing-absent/route.ts`، الكود يضع `status="غائب"` + ملاحظة "تسجيل جماعي كغائب" **قبل** فحص الإجازة. ثم `syncAcademicGradeWriteback` يكتشف الإجازة ويرمي `AcademicGradeWritebackError`، لكن السجل السيئ كان قد حُفظ. وفي بعض الحالات، الإجازة أُضيفت **بعد** التسجيل الجماعي للغياب، فالدرجة بقيت "غائب" والـ `recalculate` عاملها كغياب حقيقي.

### الإصلاح — 5 طبقات

#### الطبقة 1: `mark-missing-absent/route.ts` — تخطي المجازين قبل الإدراج
أُضيف فحص `StudentLeave` **قبل** أي إدراج للدرجة. الطلاب المجازون يُتخطون تلقائياً بدلاً من محاولة تسجيلهم كغائبين. نوعا الإجازة مدعومان:
- `leaveType="exam"` (إجازة امتحان محدد)
- `leaveType="period"` (إجازة فترة — مع التحقق من تغطية تاريخ الامتحان)

#### الطبقة 2: `syncAcademicGradeWriteback` — تحويل الحالة إلى "مجاز" بدلاً من الرفض
سابقاً: كانت ترمي `AcademicGradeWritebackError` عند كشف إجازة. هذا ترك السجل السيئ في مكانها.

الآن: إذا كانت الحالة المُرسلة `"غائب"` أو `"غش"` والطالب لديه إجازة:
- تحويل الحالة إلى `"مجاز"` (وليس رمي خطأ)
- حفظ سبب الإجازة في الملاحظة

الرفض ما زال يحدث للحالات `"درجة"` (لأن ذلك يعني إدخال درجة رقمية لطالب مجاز).

#### الطبقة 3: `sanitizeStaleAbsenceNotes` (مُحدّث)
أُضيفت معالجة `coercedToExcusedDueToLeave`:
- عند التحويل القسري من `"غائب"` إلى `"مجاز"`، استبدال الملاحظة القديمة بـ `"الطالب مجاز من هذا الامتحان: <reason>"`.
- قاعدة جديدة: إذا `status="مجاز"` + ملاحظة قديمة → استبدال بـ `"الطالب مجاز من هذا الامتحان."` (يصطاد الـ 331 صف التاريخية).

#### الطبقة 4: Migration + schema-repair — تنظيف البيانات التاريخية
- **Migration جديد:** `20260820110000_fix_excused_with_absence_notes` — ينظف 331 + 119 صف متناقض.
- **schema-repair:** نفس العبارات أُضيفت إلى `academic-schema.ts` لتُطبّق تلقائياً على كل نشر.

#### الطبقة 5: إعادة احتساب تلقائي
- بعد التنظيف، استدعيت `PATCH /api/students/academic-repair?scope=dismissed&limit=100` لإعادة احتساب الطلاب المفصولين.
- الطلاب المفصولون خطأ سيعودون إلى `"نشط"` تلقائياً لأن مصدر الفصل (درجة `"غائب"`) اختفى.

### التحقق الحيّ على الإنتاج

| المقياس | قبل | بعد |
|---|---|---|
| صفوف `status='مجاز'` + ملاحظة غياب قديمة | 331 | **0** |
| صفوف `status='غائب'/'غش'` + إجازة امتحان | 119 | **0** |
| صفوف `status='غائب'/'غش'` + إجازة فترة | 8 | **0** |
| صفوف حصلت على ملاحظة الإجازة الجديدة | 0 | **454** |
| روان ياسر (BIO-2401) | status="مجاز" + notes="تسجيل جماعي..." | status="مجاز" + notes="الطالب مجاز من هذا الامتحان." |
| حالة روان العامة | نشط (محظوظة بـ 3/3 فرص) | نشط (الفرص كاملة) |

### الملفات المُعدّلة في الإصلاح الثالث
| الملف | النوع | الوصف |
|---|---|---|
| `src/app/api/grades/mark-missing-absent/route.ts` | معدّل | تخطي المجازين قبل الإدراج |
| `src/lib/academic-grade-writeback-server.ts` | معدّل | تحويل `"غائب"` إلى `"مجاز"` بدلاً من الرفض + `sanitizeStaleAbsenceNotes` مُحدّث |
| `prisma/migrations/20260820110000_fix_excused_with_absence_notes/migration.sql` | جديد | تنظيف 450 صف متناقض تاريخياً |
| `src/lib/academic-schema.ts` | معدّل | إضافة عبارات التنظيف للتطبيق التلقائي |

### الخلاصة الكلية للإصلاحات الثلاثة
المشاكل الثلاث كانت منفصلة لكنها مترابطة منطقياً:

1. **تناقض الحالة × الدرجة** (status="غائب" + score=100) — الإصلاح الأول.
2. **تناقض الحالة × الملاحظات** (status="درجة" + notes="تسجيل جماعي كغائب") — الإصلاح الثاني.
3. **تناقض الإجازة × الغياب** (status="غائب" + إجازة فعلية) — الإصلاح الثالث (الأخطر).
